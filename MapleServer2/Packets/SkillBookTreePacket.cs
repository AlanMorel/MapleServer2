...
namespace MapleServer2.Packets {
    public static class SkillBookTreePacket {
        ...
        public static Packet Open(Player character) {
            ...
            pWriter.WriteLong(character.SkillTabs[0].Id); // Some identifier (skill tab id?)
            pWriter.WriteInt(1); // Repeat of last int and long
            pWriter.WriteLong(character.SkillTabs[0].Id); // Same as previous identifier (skill tab id?)
            pWriter.WriteUnicodeString("Build 1"); // Build name?
