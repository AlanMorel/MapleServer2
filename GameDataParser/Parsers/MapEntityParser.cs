...
namespace GameDataParser.Parsers {
    public static class MapEntityParser {
        private const string OUTPUT = "ms2-map-entity-metadata";
        ...
        public static List<MapEntityMetadata> Parse(MemoryMappedFile m2dFile, IEnumerable<PackFileEntry> entries) {
            ...
            foreach (PackFileEntry entry in entries) {
                ...
                foreach (XmlNode node in nodes) {
                    if (node.Attributes["name"].Value == "NpcList") {
                        ...
                        try {
                            ...
                            int npcId = int.Parse(npcListValue);
                        ...
                        try {
                            ...
                            string targetValue = targetNode.FirstChild.Attributes["value"].Value;
                            string portalIdValue = node.FirstChild.Attributes["value"].Value;
                            string positionValue = coordNode.FirstChild.Attributes["value"].Value;
                            string rotationValue = rotationNode?.FirstChild.Attributes["value"].Value ?? "0, 0, 0";

                            int portalId = int.Parse(portalIdValue);
                            ...
                            int target = int.Parse(targetValue);
                            CoordS position = ParseCoord(positionValue);
                            CoordS rotation = ParseCoord(rotationValue);
                ...
                // No data on this map
                if (metadata.Npcs.Count == 0 && metadata.Portals.Count == 0) {
                    continue;
                }
